  ...
  for bidx = 1:length(banknames)
    ...
    if nlFT_testWantBank(thisbankname, thistype) ...
      ...
      elseif thiscount ~= sampcount
        ...
        % Old behavior - error out if sample data length is mismatched.
%        isok = false;
...
%          thisbankname, thiscount, sampcount ));
        ...
        % New behavior - truncate to the smallest sample count found.
        warning(sprintf( [ 'Bank "%s" has %d samples, while previous ' ...
          'banks had %d samples. Field Trip doesn''t like this. ' ...
          'Truncating.' ], ...
          thisbankname, thiscount, sampcount ));
        sampcount = min(thiscount, sampcount);
